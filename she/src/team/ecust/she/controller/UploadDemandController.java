		...
		else if(goodsName.length() == 0 || goodsName.equals("方便大家了解你的需求") || goodsName.length()>=20)
		{
			...
			return;
				...
				if(demandGoodsDao.insertNewDemandGoods(demandGoods)){		
					...
					System.out.println("新上传的需求是否匹配:"+matchInfoDao.isNewDemandGoodsMatched(demandGoods));
					if(matchInfoDao.isNewDemandGoodsMatched(demandGoods)){
						new PromptBox(Tips.TICK).open("上传且匹配成功! 请去看看你的匹配!");
							matchInfoDao.insertNewDemandMatchIdleInfo(demandGoods);
					}
					else{
